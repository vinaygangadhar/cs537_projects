    ...
    while (1) 
		{
	    	...
	    	printf("                            	SERVER:: Read %d bytes (message: '%s')\n", rc, buffer);
				...
				int pinum, inum, type, block, entries, i;
				char name[NAME_SIZE];
				...
				switch(ret_func)
				{
					...
					case 4:
						...
						if(!ret)
							res_t = S_Read(inum, (char*)&rw_buf, block, &type, &entries);
						else
							res_t = -1;
						...
						char temp[3];
						memset(buf, 0, MSG_BUFFER_SIZE);
						snprintf(temp, 3*sizeof(int) , "%d%d%d", ret_func, res_t, type);
						
						for(i = 0; i < 3; i++)
							buf[i] = temp[i];
					
						printf("Read server side: ");
						for(i = 0; i < 4096; i++)
						{	
							buf[3+i] = rw_buf[i];
							printf("%c", buf[3+i]);
	    			...
	    			printf("                            	SERVER:: Will ShutDown\n");
						...
						//MSG Format: Function_id, Success_state
						memset(buf, 0, MSG_BUFFER_SIZE);
						snprintf(buf, 2*sizeof(int) , "%d%d", ret_func, 0);
						send_resp(sd, &s, buf);
