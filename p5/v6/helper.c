...
int S_Creat(int pinum, int type, char *name)
{
	...
	{
	/*
		//Check if the type of found file is same as type sent
		int check_inode = DB_Iter.entry[entrynum].inum;
		CR_t cr;
		inode_t in;
		imap_t imp;

		ret = CR_to_Inode(check_inode, &cr, &imp, &in);
		if(ret < 0)
			return -1;
...
{
	//Check if the parent directory contains requested "name"
	if(S_Lookup(pinum, name) == -1)
		return 0;

#ifdef DEBUG
		printf("File/Directory exists in the parent directory\n");
#endif
	...
	for(it = 0; it < MAX_DIRECT_P; it++)
	{
...
#ifdef DEBUG
		printf("Found the file in DB %d and the Inode entry: %d\n", it, entrynum);
#endif

		//Get the inode information of the found inode
		int unlink_inum = DB_Iter.entry[entrynum].inum;
		inode_t inode_del;
		imap_t imap_del;

		ret = CR_to_Inode(unlink_inum, &CR, &imap_del, &inode_del);
				
		//Check if the unlinking inode is directory
#ifdef DEBUG
		printf("Inum: %d Type: %d Size: %d\n", unlink_inum, inode_del.type, inode_del.size);
#endif
		
	
		//checking for entries in the dir
		if(inode_del.type == MFS_DIRECTORY)
		{
			Dir_t DB_check;
			uint dblk_addr;
			...
			{
				entryno = 0;
				dblk_addr = inode_del.dp[iter];  													//Need to check if the db_addr is valid
				if(dblk_addr == 0)
					continue;

				ret = db_read(dblk_addr, &DB_check);
				if(ret < 0)
					return -1;
				...
				for(entryno = 0; entryno < MAX_DIR_ENTRIES; entryno++)
				{
					DirEnt_t check_entry = DB_check.entry[entryno];
					if(check_entry.inum != -1)
					{
						found_count += 1;
					}
				}
		...
		//Update the entry inode num to -1
		DB_Iter.entry[entrynum].inum = -1;

#ifdef DEBUG
		printf("Removing the inode name: %s and inum: %d\n", DB_Iter.entry[entrynum].name, DB_Iter.entry[entrynum].inum);	
#endif
		...
		//Write the parent DB
		lseek(disk_fd, cur_addr, SEEK_SET);
		bw = write(disk_fd, &DB_Iter, sizeof(Dir_t)); 
		if(bw != sizeof(Dir_t))
			perror("write");

		//Write inode of parent
		parent_inode.dp[it] = cur_addr;
		...
		//Write the imap of the deleted inode
		cur_addr += sizeof(imap_t);

		int del_imap_num = (int)(unlink_inum / 16);
		int del_imap_off = unlink_inum  % 16;

		if(pimap_num == del_imap_num)
		{
			printf("Parent and the deleted inode's imap are same\n");
			imap_del = parent_imap;
		}
	
		imap_del.inode_p[del_imap_off] = 0; // make that inode offset in the imap as 0
		
		lseek(disk_fd, cur_addr, SEEK_SET);
		bw = write(disk_fd, &imap_del, sizeof(imap_t));
		if(bw != sizeof(imap_t))
			perror("write");


		//Finally updat the CR
		CR.imap_p[del_imap_num]= cur_addr;
		cur_addr += sizeof(imap_t);
