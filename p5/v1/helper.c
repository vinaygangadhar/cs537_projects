...
{
	int new_fd;
	new_fd = open(file_path, O_RDWR | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR);
	if(new_fd == -1)
...
//	const char* root_name  = "/";
	...
	bw = write(new_fd, crp, sizeof(CR_t));
	if(bw != sizeof(CR_t))
		perror("write");
	...
	bw = write(new_fd, rp, sizeof(Dir_t));
	if(bw != sizeof(Dir_t) )
		perror("write");
	...
	bw = write(new_fd, ip, sizeof(inode_t));
	...
	bw = write(new_fd, imp, sizeof(imap_t));
	if(bw != sizeof(imap_t) )
		perror("write");
